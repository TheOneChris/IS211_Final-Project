...
def search_form():
    book_list = main()
    return render_template("booksearch.html", books=book_list)
...
def add():
    ...
    averageRating = None
    if "averageRating" in (obj["items"][0]["volumeInfo"]):
        averageRating = (obj["items"][0]["volumeInfo"]["averageRating"])
    thumbnail = (obj["items"][0]["volumeInfo"]["imageLinks"]["thumbnail"])

    conn = sqlite3.connect("database.db")
    c = conn.cursor()
    sql = "INSERT INTO books (ISBN, Title, Authors, PageCount, AverageRating, Thumbnail) VALUES (?, ?, ?, ?, ?, ?)"
    val = (ISBN, str(title), str(authors), str(pageCount), str(averageRating), str(thumbnail))
